...
class Section: 
    ...
    def func(self, phi, v_inf, omega):
        ...
        denom_a  = (1 + C * a)
        denom_ap = (1 - C * ap)
        denom_a  = denom_a  if abs(denom_a)  > 1e-6 else (1e-6 if denom_a  >= 0 else -1e-6)
        denom_ap = denom_ap if abs(denom_ap) > 1e-6 else (1e-6 if denom_ap >= 0 else -1e-6)
        resid = sin(phi)/denom_a - v_inf*cos(phi)/(omega*self.radius*denom_ap)
